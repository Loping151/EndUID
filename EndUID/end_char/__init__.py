...
async def refresh_card_detail_handler(bot: Bot, ev: Event):
    ...
    if not uid:
        return await bot.send("❌ 未绑定终末地账号，请先绑定")

    _, cred = await end_api.get_ck_result(uid, ev.user_id, ev.bot_id)
    if not cred:
        return await bot.send("❌ 未找到可用凭证，请重新绑定")
    ...
    if not res:
        return await bot.send("❌ 刷新失败：请求卡片详情失败")

    if res.get("code") != 0:
        message = res.get("message", "未知错误")
        return await bot.send(f"❌ 刷新失败: {message}")
    ...
    try:
        ...
        return await bot.send("❌ 刷新失败：保存卡片详情失败")

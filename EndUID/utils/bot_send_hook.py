...
_end_manager = get_or_create_hook_manager("End")


def register_target_send_hook(func: Callable):
    """注册 target_send 方法 hook"""
    _end_manager.register_target_send_hook(func)
...
async def _call_all_user_activity_hooks(
    user_id: Optional[str], bot_id: str, bot_self_id: str
):
    """调用所有插件的用户活跃度 hooks"""
    if not user_id:
        return
...
def install_bot_hooks():
    ...
    async def hooked_send(self, *args, **kwargs):
        ...
        bot_self_id = (
            getattr(self, "bot_self_id", "") if hasattr(self, "bot_self_id") else ""
        )
    ...
    async def hooked_target_send(self, *args, **kwargs):
        ...
        if hasattr(self, "ev") and len(args) >= 3:
            ...
            target_id = args[2]
            bot_id = getattr(
                self.ev, "real_bot_id", getattr(self, "bot_id", "")
            )
            bot_self_id = getattr(
                self.ev, "bot_self_id", getattr(self, "bot_self_id", "")
            )

            await _call_all_target_send_hooks(
                target_type, target_id, bot_id, bot_self_id
            )
